<script>
  ...
  $: isStaticRoute =
    $store.staticRoutes.filter((r) => $page.url.pathname.startsWith(r.path))
      ?.length > 0;
...
*/
  ...
  $: spaceTitle = data?.space?.alias
    ? `${data?.space?.alias} -
${PUBLIC_META_TITLE}`
    : PUBLIC_META_TITLE;

  $: fb = data?.event?.content?.body
    ? `${data?.event?.content?.body.slice(0, 1000)}...`
    : "";

  $: pb =
    data?.event?.content?.body?.length > 50
      ? `${data?.event?.content?.body.slice(0, 50)}...`
      : data?.event?.content?.body;

  $: pt = data?.event?.content?.title
    ? data?.event?.content?.title
    : pb
    ? pb
    : "";

  $: ct = data?.event?.type == "m.room.message" ? `${sender}: "${pt}"` : pt;
  ...
  $: hasImage =
    data?.event?.content?.attachments?.length > 0 &&
  ...
  $: sender = data?.event?.sender?.display_name
    ? data?.event?.sender?.display_name
    : data?.event?.sender?.username;
